...
for f in Rehab Kcal BodyParse Sentence Days Hu Progression Muscles Mobility Alarms Activities StrengthParse Examples Load MealIdeas IntervalParse Weekplan Bests TimeHint Habits Warmup Routines; do
...
# A Sleep tárolása Context-es; a tesztekhez csak a tiszta rész kell.
python3 - "$SRC/Sleep.java" "$PKG/Sleep.java" <<'PY'
import sys
lines = open(sys.argv[1]).read().split('\n')
out = []; i = 0
while i < len(lines):
